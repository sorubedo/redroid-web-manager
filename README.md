...
一个跑在本机上的 redroid(Android in Docker)管理台:建容器、拉镜像、做定制镜像、启停删除、装 APK,都在浏览器里点完,不用记 docker 命令。最省事的是「看屏幕」—— 直接在网页里操作 Android 画面和声音,不用装 adb 客户端,也不用另开一个窗口。

![容器列表与网页里的 Android 画面](docs/demo.png)
...
| 命令行 | 环境变量 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `--host <地址>` | `REDROID_WEB_HOST` | `127.0.0.1` | 监听地址 |
...
|  | `REDROID_WEB_ADB_HOST` | `REDROID_WEB_HOST` | 去哪台机器连容器的 adb 端口 |
